...
write.csv(statisticsByCountryEF, "output_statistics_by_country_exploratory.csv")
...
# Save the results to a CSV file
write.csv(summaryFitIC, "model_fit_by_country_exploratory.csv")
...
#results from the MMGFA analysis (shortcut to save time). To replicate the MMGFA analysis, remove the '' brackets below
djg_clusters_list <- list(
  cluster_a = c(1, 2, 4, 5, 6, 7, 10, 11, 13, 14, 15, 22, 23, 24),
  cluster_b = c(3, 9, 12, 19, 20),
...
# Save the results to a CSV file
write.csv(djgMmgResultsEF, "clusters_invariance_exploratory.csv", row.names = FALSE)
...
# Define clusters with names (hardcoded variables taken from the MMGFA output)
clusters <- list(
  cluster_b = c(3, 9, 12, 19, 20),
  cluster_c = c(8, 16, 17, 18, 21, 25, 26, 27)
)
...
write.csv(djgGenderAgeResults, "clusters_invariance_exploratory_gender_age.csv", row.names = FALSE)
...
#'### T-ILS invariance results for country, gender, and age
...
# Save and print
write.csv(tilsInvarianceDF, "tils_invariance_exploratory.csv", row.names = FALSE)
...
write.xlsx(combined_proportions_df, "nomologicalNetCorr_exploratory.xlsx")
